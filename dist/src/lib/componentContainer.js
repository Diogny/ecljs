...
class ComponentContainer {
    ...
    get itemList() { return Array.from(this.itemMap.values()).map(item => item.c); }
    get wireList() { return Array.from(this.wireMap.values()).map(item => item.c); }
    get empty() { return !(this.wireMap.size || this.itemMap.size); }
    get components() { return this.itemList.concat(this.wireList); }
    ...
    selectAll(value) {
        return this.selectedComponents = Array.from(this.itemMap.values())
    ...
    toggleSelect(comp) {
        ...
        this.selectedComponents =
            this.itemList.filter(c => c.selected);
    }
    selectThis(comp) {
        return comp
    ...
    destroy() {
        this.itemList.forEach(ec => this.delete(ec));
...
function createBoardItem(container, options) {
    ...
    newComp
        && (base.count = base.comp.meta.countStart | 0, container.compMap.set(name, base));
