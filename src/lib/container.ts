...
export abstract class Container<T extends ItemBoard> extends Base {
  ...
  public itemBonds(item: T | Wire): Bond[] | undefined {
    ...
    return this.$.itemMap.get(item.id)?.b || this.$.wireMap.get(item.id)?.b;
  }
...
function getBaseComp<T extends ItemBoard>(that: Container<T>, $: IContainerDefaults<T>, name: string) {
  let
    obj: { [x: string]: any; } = {
 ...
 */
function createBoardItem<T extends ItemBoard>(that: Container<T>, $: IContainerDefaults<T>, options: { [x: string]: any; }): T | Wire {
  let
    base: IBaseComponent = <any>void 0,
    item: T | Wire = <any>void 0,
    setBase = () => {
      if (!(base = <IBaseComponent>that.root(options.name))) {
        base = <IBaseComponent>getBaseComp(that, $, options.name);
