...
export class ItemSolid extends ItemBoard {
    ...
    rect() {
        ...
        if (this.rot) {
            ...
            let origin = this.origin, angle = -this.rot, points = [[p.x, p.y], [p.x + size.width, p.y], [p.x, p.y + size.height], [p.x + size.width, p.y + size.height]]
                .map(p => Point.rotateBy(p[0], p[1], origin.x, origin.y, angle)), x = Math.min.apply(Math, points.map(a => a.x)), y = Math.min.apply(Math, points.map(a => a.y)), w = Math.max.apply(Math, points.map(a => a.x)), h = Math.max.apply(Math, points.map(a => a.y));
     ...
     */
    node(node, nodeOnly) {
        let pin = pinInfo(this.base.meta.nodes.list, node);
        if (!pin)
            return;
        if (!nodeOnly) {
            if (this.rot) {
                let center = this.origin, rot = Point.rotateBy(pin.x, pin.y, center.x, center.y, -this.rot);
                pin.x = rot.x;
                pin.y = rot.y;
            }
            pin.x += this.x;
